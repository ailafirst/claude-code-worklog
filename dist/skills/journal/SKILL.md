...
- **没有 git 证据的改动，写之前必须核实**：gitignored 文件、未跟踪文件的改动，`collect`
  拿不到 diff 佐证——凭对话记忆写「做成了什么」最容易把「讨论过/考虑过」和「真的落地了」
  搞混。这类改动落笔前，先用 Read/Grep 重新看一眼那个文件当前内容，确认真的写进去了；
  核实不了就别写这条，或改成「讨论过 XX，未落地」，不能凭记忆断言。
...
- **少用 AI 味装饰**：五个槽位标题用模板里的 `### ` 三级标题，不要改成 `**加粗**`；
  正文里也别堆加粗、emoji、"首先/其次/总之"这类套话——像人手写的工作笔记，不是汇报稿。
...
✅/❌ 没有 git 证据的改动（gitignored 文件是重灾区）：
> session 里讨论过"要不要给 CLAUDE.md 加一条 .gitattributes 的坑"，但最终没有实际编辑该文件。
> ✅ 应写：「讨论过给 CLAUDE.md 补一条 eol=lf 的坑，未落地」或干脆不提。
>
> ❌ 反例：直接写「唯一改动：CLAUDE.md 补了一条 .gitattributes 强制 eol=lf 的坑」——
> 该文件被 gitignore，`collect` 没有 diff 能证伪，凭记忆写等于编造，
> 而 CLAUDE.md 是否真被改过，Read 一下当前内容就能核实，没有理由跳过这一步。
