...
| 用户说 | 对应模式 |
|--------|---------|
| "记一下这次 session""日记" | `/journal` → capture |
| "open thread""线程停了没" | `/journal threads` |
| "周 rollup""总结这周" | `/journal rollup` |
| "随手记一下 …" | `! py … journal.py note -m "…"` |

**collect 与 append 必须在当前工作 git 仓目录下运行**，脚本才看得到 git、才能回填 `head`。

## capture 工作流（无参数 `/journal`）
...
```
! py ~/.claude/skills/journal/journal.py note -m "某个绕过方案值得试"
```
...
若已配置 Stop hook，每次 Claude Code 退出时自动运行：

```
py ~/.claude/skills/journal/journal.py snapshot
```
...
"hooks": {
  "Stop": [{
    "matcher": "",
    "hooks": [{"type": "command",
               "command": "py C:\\Users\\PC\\.claude\\skills\\journal\\journal.py snapshot"}]
  }]
}
```

或运行 `py install.py --hook` 自动写入。
